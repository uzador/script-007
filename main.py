...
def default_cmd(cmd):
    ...
    raise exception.UnknownCommandException('Unknown command received')
...
def process(cmd, file_name, content):
    ...
    except exception.UnknownCommandException as ex:
        logger.error(f'UnknownCommand: {ex.msg}')
    except exception.SignatureException as ex:
        logger.error(f'SignatureException: {ex.msg}')
